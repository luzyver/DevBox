...
export function PickerModal<T>({ open, title, items, keyOf, labelOf, onSelect, onCancel, isLocked, renderActions }: Props<T>) {
  if (!open) return null
      ...
      <div className="relative bg-card border-2 border-foreground rounded-xl shadow-pop max-w-md w-full animate-pop-in max-h-[70vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2">
          ...
          {items.map(item => {
            ...
            return (
              <div key={keyOf(item)} className={`flex items-center gap-1 px-3 py-2 rounded-lg hover:bg-muted transition-colors group ${locked ? 'bg-secondary/10 border-l-2 border-secondary' : ''}`}>
